...
export default function ProfilePage() {
  ...
  function handleAvatarClick() {
    if (profile?.avatar_url) {
      // Has avatar — ask to remove first or change
      setConfirmAction({
      ...
      // No avatar — open file picker directly
      ...
      <div className="lg:sticky lg:top-20 lg:self-start space-y-4">
        ...
        <div className="rounded-xl bg-card ring-1 ring-foreground/[0.06] shadow-card overflow-hidden">
          ...
          <div className="px-4 pb-4 -mt-10 flex flex-col items-center">
            ...
            <p className="text-[11px] text-muted-foreground">{profile.block && profile.lot ? `${profile.block}, ${profile.lot}` : 'Homeowner'}</p>
            {!profile.avatar_url && (
              <button
        ...
        <div className="hidden lg:block rounded-xl bg-card ring-1 ring-foreground/[0.06] shadow-card p-4 space-y-3">
          ...
          <div className="flex items-center justify-between text-[12px]">
            ...
            <span className="font-medium">{profile.block && profile.lot ? `${profile.block}, ${profile.lot}` : '—'}</span>
          </div>
        </div>
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    ...
                    <Input value={profile.block || 'N/A'} disabled className="bg-muted/50" />
                  </div>
                  <div className="space-y-1.5">
                    <Label className="text-sm text-muted-foreground">Lot</Label>
                    <Input value={profile.lot || 'N/A'} disabled className="bg-muted/50" />
                  </div>
                </div>
